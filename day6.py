...
directives = ['turn off 660,55 through 986,197',
...
class Directive():
    def __init__(self, op, x0, y0, x1, y1):
        self.op = re.sub('turn ', '', op)
        self.x0 = int(x0)
        self.x1 = int(x1)
        self.y0 = int(y0)
        self.y1 = int(y1)

##  here's the array, '-' means off, '+' means on
ary = [[0] * 1000 for j in range(1000)]

##directives = ['turn on 0,0 through 999,999']
##directives = ['turn on 0,0 through 0,0']
##directives = ['toggle 0,0 through 999,999']

pat = r'(turn on|turn off|toggle) ([0-9]*),([0-9]*)[^0-9]*([0-9]*),([0-9]*)'
for i in range(len(directives)):
    t = re.match(pat, directives[i]).groups()
    d = Directive(*t)
    for x in range(d.x0, d.x1+1):
        for y in range (d.y0, d.y1+1):
            if d.op == 'on':
                ...
                ary[x][y] -= 1
                ary[x][y] = 0 if ary[x][y] < 0 else ary[x][y]
            else:
                ary[x][y] += 2
...
## correct answer: '15343601 lights are lit'
print "%d lights are lit" % lit
